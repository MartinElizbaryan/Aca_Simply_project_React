...
import Question from '../Shared/Quetions/Quetion/Quetion'
export default function CreatePost() {
    const [quetion, setQuetion] = useState([])
    const classes = useStyles();
    const addQuetion = () => {
        let newQuetion;
        newQuetion = [...quetion, <Question key={quetion.length + 2 - 1} />];
        setQuetion(newQuetion)
    }
    return (
            ...
            <Grid item xs={12} md={9} mt={6}>
                <Box mt={5} mb={5}>
                    <Grid container spacing={2} p={2}>
                        ...
                        <Grid item xs={12}>
                            <TextField className={classes.input} fullWidth label="Type" variant="outlined" size='normal' select value="lost" >
                                <MenuItem value="lost">
                                    Lost
                                </MenuItem>
                        ...
                        <Grid item xs={12}>
                            <TextField className={classes.input} fullWidth label="Description" variant="outlined" size='normal' multiline minRows={5} maxRows={10} />
                        </Grid>
                        <Grid item xs={12} md={6} lg={4} display="flex">
                            <UploadButtons />
                        </Grid>
                        <Grid item xs={12} md={6} lg={12} display="flex" alignItems="center">
                            <AddButton onClick={addQuetion} /> <Typography variant="span" ml={3}>Add Question</Typography>
                        </Grid>
                    </Grid>
                    {quetion}
                    <Grid container spacing={2} p={2}>
